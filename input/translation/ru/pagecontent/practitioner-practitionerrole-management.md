...
### Понимание Practitioner и PractitionerRole

**Practitioner** и **PractitionerRole** — это отдельные, но связанные ресурсы:

- **Practitioner**: Представляет человека — его демографические данные, квалификации и личные идентификаторы. Практикующий специалист создаётся один раз и содержит информацию, которая не меняется в зависимости от того, где или как он работает.
  - Пример: Д-р Алишер Каримов с национальным идентификатором, датой рождения, квалификациями

- **PractitionerRole**: Представляет работу или должность — роли, специальности и места, где практикующий специалист работает. Один и тот же практикующий специалист может иметь несколько ролей в разных организациях или отделениях.
  - Пример: Д-р Каримов работает терапевтом в Клинике А и консультантом в Больнице Б

**Почему отдельные ресурсы?**
- Врач может работать в нескольких организациях одновременно
- Врач может иметь разные роли в одной организации (например, хирург и заведующий отделением)
- Информация, специфичная для роли (график, контактные телефоны, места) относится к роли, а не к человеку
- Назначения на работу меняются чаще, чем личная информация

**Реальный сценарий:**
1. Создать один ресурс Practitioner для д-ра Каримова
2. Создать PractitionerRole #1: Д-р Каримов как терапевт в Ташкентской клинике
3. Создать PractitionerRole #2: Д-р Каримов как консультант по кардиологии в Ферганской больнице
4. Если д-р Каримов прекращает работу в Ташкентской клинике, деактивировать PractitionerRole #1, не трогая ресурс Practitioner или PractitionerRole #2
...
3. **Получение данных практикующего специалиста** для каждой роли:
   ```
   GET /Practitioner/practitioner-id
   ```

4. **Получение квалификаций** из ресурса Practitioner

5. **Получение дополнительных данных роли** при необходимости:
   ```
